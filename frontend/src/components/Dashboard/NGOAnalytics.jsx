...
export default function NGOAnalytics({ profile }) {
    ...
    const [comparisonNGO, setComparisonNGO] = useState(null);
    const [comparisonData, setComparisonData] = useState(null);
    const [ngosList, setNgosList] = useState([]);
    const [loadingComparison, setLoadingComparison] = useState(false);

    useEffect(() => {
        // Get NGO ID from profile or localStorage
    ...
    useEffect(() => {
        if (comparisonMode && comparisonNGO) {
            fetchComparisonData();
        } else {
            setComparisonData(null);
        }
    }, [comparisonMode, comparisonNGO]);
    ...
    const fetchAnalytics = async () => {
        ...
        try {
            ...
            const response = await axios.get(`${API_BASE_URL}/ngos/${ngoId}/analytics`);
    ...
    const fetchNGOsList = async () => {
        try {
            const response = await axios.get(`${API_BASE_URL}/ngos/list`, {
                params: {
                    ngoType: profile?.ngoType || null,
                    city: profile?.city || null
                }
            });
            // Filter out current NGO from the list
            const filtered = response.data.filter(n => n.id !== profile?.id);
            setNgosList(filtered);
        } catch (error) {
            console.error("Error fetching NGOs list:", error);
        }
    };

    const fetchComparisonData = async () => {
        if (!comparisonNGO?.id) return;
        try {
            setLoadingComparison(true);
            const response = await axios.get(`${API_BASE_URL}/ngos/${comparisonNGO.id}/analytics`);
    ...
    const ngoId = profile?.id || parseInt(localStorage.getItem("userId"));
    if (!ngoId) {
        return (
            <div className="flex items-center justify-center h-full">
        ...
        <div className="space-y-6 animate-fade-in p-4">
            <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-serif font-bold text-gray-900 dark:text-white">
                    {comparisonMode ? 'NGO Comparison Dashboard' : 'NGO Analytics Dashboard'}
                </h2>
                <div className="flex items-center gap-4">
                    ...
                    {comparisonMode && (
                        <select
                            value={comparisonNGO?.id || ''}
                            onChange={(e) => {
                                const selected = ngosList.find(n => n.id === parseInt(e.target.value));
                                setComparisonNGO(selected || null);
                            }}
                        ...
                        >
                            <option value="">Select an NGO to compare...</option>
                            {ngosList.map((ngo) => (
                                <option key={ngo.id} value={ngo.id}>
                                    {ngo.ngoName} - {ngo.ngoType} ({ngo.city})
                                </option>
                            ))}
                        </select>
                    )}
                                ...
                                <div className="flex items-center gap-6">
                                    <div>
                                        ...
                                        <p className="text-base font-semibold text-gray-900 dark:text-white">
                                            {comparisonNGO.ngoName}
                                        </p>
                                        <p className="text-xs text-gray-500 dark:text-gray-500">
                                            {comparisonNGO.ngoType} • {comparisonNGO.city}, {comparisonNGO.state}
                                        </p>
                                    </div>
                                    {comparisonNGO.verificationStatus && (
